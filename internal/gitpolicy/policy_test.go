...
func branchListed(entries []gitpolicy.LocalBranchEntry, name string, protected bool) bool {
	for _, entry := range entries {
		if entry.Name == name && entry.IsProtected == protected {
			return true
		}
	}
	return false
...
func refListed(entries []gitpolicy.LocalRefEntry, ref, kind string) bool {
	for _, entry := range entries {
		if entry.Ref == ref && entry.Kind == kind {
			return true
		}
	}
	return false
...
func pathStatusEntry(entries []gitpolicy.PathStatusEntry, path string) *gitpolicy.PathStatusEntry {
